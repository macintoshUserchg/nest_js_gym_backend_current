...
export class CreatePaymentDto {
  ...
  @ApiProperty({ description: 'Payment method', example: 'card', enum: ['cash', 'card', 'online', 'bank_transfer'] })
  @IsEnum(['cash', 'card', 'online', 'bank_transfer'])
  @IsNotEmpty()
  method: string;

  @ApiPropertyOptional({ description: 'Reference number', example: 'TXN123456' })
  @IsString()
  @IsOptional()
  referenceNumber?: string;

  @ApiPropertyOptional({ description: 'Payment notes', example: 'Paid via credit card' })
  @IsString()
  @IsOptional()
  notes?: string;
}
