...
export class WorkoutsController {
  ...
  @ApiOperation({ 
    ...
    description: 'Creates detailed workout plans and templates including structured exercise routines, progressive overload schedules, target muscle groups, equipment requirements, and coaching notes. This endpoint allows trainers to design comprehensive training programs for member fitness goals.'
  })
  @ApiResponse({
    status: 201,
    ...
    schema: {
      ...
      properties: {
        ...
        planName: { type: 'string', example: 'Strength Building Program - Phase 1' },
        planType: { type: 'string', example: 'strength', enum: ['strength', 'cardio', 'flexibility', 'hiit', 'sports_specific', 'rehabilitation', 'general_fitness'] },
        duration: { type: 'number', example: 12 }, // weeks
        durationUnit: { type: 'string', example: 'weeks', enum: ['days', 'weeks', 'months'] },
        difficulty: { type: 'string', example: 'intermediate', enum: ['beginner', 'intermediate', 'advanced', 'expert'] },
        sessionsPerWeek: { type: 'number', example: 4 },
        estimatedDuration: { type: 'number', example: 60 }, // minutes per session
        goal: { type: 'string', example: 'Build lean muscle mass and increase strength' },
        description: { type: 'string', example: 'A comprehensive 12-week program focusing on compound movements and progressive overload' },
        exercises: {
          type: 'array',
          items: {
            type: 'object',
            properties: {
              ...
              category: { type: 'string', example: 'compound', enum: ['compound', 'isolation', 'functional'] },
              muscleGroups: {
                type: 'array',
                items: { type: 'string' },
              ...
              sets: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    weightUnit: { type: 'string', example: 'kg', enum: ['kg', 'lbs', 'bodyweight', 'time_based'] },
                    restTime: { type: 'number', example: 120 }, // seconds
                    tempo: { type: 'string', example: '3-1-1', description: 'Down-Pause-Up time in seconds' },
                    notes: { type: 'string', example: 'Focus on controlled descent and full range of motion' }
                  }
                }
              },
              progression: {
                type: 'object',
                properties: {
                  type: { type: 'string', example: 'weight', enum: ['weight', 'reps', 'volume', 'time', 'complexity'] },
                  frequency: { type: 'string', example: 'weekly', enum: ['every_session', 'weekly', 'bi_weekly', 'monthly'] },
                  increment: { type: 'number', example: 2.5 },
                  description: { type: 'string', example: 'Increase weight by 2.5kg each week if all sets completed' }
                }
              },
              ...
              alternatives: {
                type: 'array',
                items: { type: 'string' },
                example: ['Dumbbell Bench Press', 'Push-ups', 'Chest Press Machine']
              }
            }
          }
        },
        schedule: {
          ...
          items: {
            type: 'object',
            properties: {
              day: { type: 'string', example: 'monday', enum: ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] },
              sessionType: { type: 'string', example: 'upper_body', enum: ['upper_body', 'lower_body', 'full_body', 'cardio', 'rest', 'flexibility'] },
              duration: { type: 'number', example: 60 },
              focus: { type: 'string', example: 'Push movements - chest, shoulders, triceps' }
            }
          }
        },
        warmup: {
        ...
        nutrition: {
          type: 'object',
          properties: {
            recommendations: { type: 'string', example: 'Increase protein intake to 1.6g per kg body weight' },
            timing: { type: 'string', example: 'Consume protein within 30 minutes post-workout' },
            supplements: {
              type: 'array',
              items: { type: 'string' },
        ...
        trackingMetrics: {
          type: 'array',
          items: { type: 'string' },
          example: ['weight_progression', 'body_measurements', 'strength_tests', 'endurance_metrics']
        },
        modifications: {
          type: 'array',
          items: { type: 'string' },
          example: ['Reduce weight if form breaks down', 'Modify range of motion for injuries', 'Substitute exercises for equipment limitations']
        },
        safetyNotes: {
          type: 'array',
          items: { type: 'string' },
          example: ['Always use safety spotters', 'Stop if experiencing pain', 'Maintain proper form over heavy weights']
        },
        status: { type: 'string', example: 'active', enum: ['active', 'completed', 'paused', 'cancelled'] },
  ...
  @ApiResponse({ status: 400, description: 'Invalid workout plan data provided or validation failed.' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to create workout plans.' })
  @ApiResponse({ status: 404, description: 'Member not found or does not exist.' })
  @ApiBody({ 
    type: CreateWorkoutPlanDto,
    description: 'Workout plan structure including exercises, sets, progressions, and coaching guidelines',
    examples: {
      strength_program: {
        ...
        value: {
          ...
          schedule: [
            { day: 'monday', sessionType: 'upper_body', duration: 60, focus: 'Push movements' },
            { day: 'tuesday', sessionType: 'lower_body', duration: 60, focus: 'Legs and glutes' },
            { day: 'thursday', sessionType: 'upper_body', duration: 60, focus: 'Pull movements' },
            { day: 'friday', sessionType: 'full_body', duration: 60, focus: 'Compound movements' }
          ],
              ...
              sets: [
                {
                  ...
                  notes: 'Maintain moderate intensity, alternate between brisk walk and jog'
                }
              ],
              progression: {
                type: 'time',
                frequency: 'bi_weekly',
                increment: 5,
                description: 'Increase session duration by 5 minutes every 2 weeks'
              }
            }
          ],
          schedule: [
            { day: 'monday', sessionType: 'cardio', duration: 45, focus: 'Steady state cardio' },
            { day: 'wednesday', sessionType: 'cardio', duration: 45, focus: 'Interval training' },
            { day: 'friday', sessionType: 'cardio', duration: 45, focus: 'Mixed intensity' }
          ],
          trackingMetrics: ['endurance_metrics', 'heart_rate_zones', 'calorie_burn']
        }
      }
    }
  })
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves comprehensive list of workout plans and templates with advanced filtering, analytics, and program insights. Supports filtering by plan type, difficulty, target goals, duration, and creator. Provides program performance metrics and member success rates.'
  })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number for pagination (default: 1)', example: 1 })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Number of plans per page (default: 20, max: 100)', example: 20 })
  @ApiQuery({ name: 'memberId', required: false, type: Number, description: 'Filter by specific member ID', example: 123 })
  @ApiQuery({ name: 'planType', required: false, type: String, description: 'Filter by plan type', example: 'strength', enum: ['strength', 'cardio', 'flexibility', 'hiit', 'sports_specific', 'rehabilitation', 'general_fitness'] })
  @ApiQuery({ name: 'difficulty', required: false, type: String, description: 'Filter by difficulty level', example: 'intermediate', enum: ['beginner', 'intermediate', 'advanced', 'expert'] })
  @ApiQuery({ name: 'status', required: false, type: String, description: 'Filter by plan status', example: 'active', enum: ['active', 'completed', 'paused', 'cancelled'] })
  @ApiQuery({ name: 'isTemplate', required: false, type: Boolean, description: 'Show only template plans', example: false })
  @ApiQuery({ name: 'creatorId', required: false, type: Number, description: 'Filter by plan creator', example: 45 })
  @ApiQuery({ name: 'durationMin', required: false, type: Number, description: 'Minimum duration in weeks', example: 4 })
  @ApiQuery({ name: 'durationMax', required: false, type: Number, description: 'Maximum duration in weeks', example: 24 })
  @ApiQuery({ name: 'sessionsPerWeek', required: false, type: Number, description: 'Filter by sessions per week', example: 3 })
  @ApiQuery({ name: 'sortBy', required: false, type: String, description: 'Sort field', example: 'createdAt', enum: ['createdAt', 'planName', 'duration', 'difficulty', 'sessionsPerWeek'] })
      ...
      properties: {
        ...
        analytics: {
          ...
          properties: {
            ...
            mostRequestedDifficulty: { type: 'string', example: 'intermediate' },
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to view workout plans.' })
  findAll() {
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves detailed information about a specific workout plan including complete exercise definitions, progression schedules, member assignments, performance tracking, and program analytics. Provides comprehensive view of the training program structure and effectiveness.'
  })
  @ApiParam({ 
    name: 'id', 
  ...
  @ApiResponse({ 
    ...
    schema: {
      ...
      properties: {
        ...
        sessionsPerWeek: { type: 'number', example: 4 },
        estimatedDuration: { type: 'number', example: 60 },
        goal: { type: 'string', example: 'Build lean muscle mass and increase overall strength by 20%' },
        description: { type: 'string', example: 'A comprehensive 12-week program focusing on compound movements and progressive overload' },
        status: { type: 'string', example: 'active', enum: ['active', 'completed', 'paused', 'cancelled'] },
        isTemplate: { type: 'boolean', example: false },
        startDate: { type: 'string', format: 'date-time', example: '2024-01-15T00:00:00.000Z' },
        targetEndDate: { type: 'string', format: 'date-time', example: '2024-04-08T00:00:00.000Z' },
        ...
        exercises: {
          ...
          items: {
            ...
            properties: {
              ...
              difficulty: { type: 'string', example: 'intermediate', enum: ['beginner', 'intermediate', 'advanced'] },
              sets: {
                type: 'array',
                items: {
                  type: 'object',
              ...
              memberPerformance: {
                ...
                properties: {
                  ...
                  bestPerformance: { type: 'string', example: '70kg x 12 reps' },
        ...
        createdAt: { type: 'string', format: 'date-time', example: '2024-01-15T10:30:00.000Z' },
        updatedAt: { type: 'string', format: 'date-time', example: '2024-01-20T14:20:00.000Z' }
      }
    }
  })
  @ApiResponse({ status: 400, description: 'Invalid workout plan ID format.' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to view this workout plan.' })
  @ApiResponse({ status: 404, description: 'Workout plan not found or does not exist.' })
