...
export class ProgressTrackingController {
  ...
  @ApiOperation({ 
    ...
    description: 'Records detailed progress tracking data for gym members including performance metrics, milestone achievements, workout completion rates, and fitness goal progress. This endpoint allows trainers to monitor member development and track fitness journey milestones.'
  })
  @ApiResponse({
    status: 201,
    ...
    schema: {
      ...
      properties: {
        ...
        goalType: { type: 'string', example: 'weight_loss', enum: ['weight_loss', 'muscle_gain', 'endurance', 'strength', 'flexibility', 'general_fitness'] },
        currentValue: { type: 'number', example: 75.5 },
        targetValue: { type: 'number', example: 70.0 },
        unit: { type: 'string', example: 'kg', enum: ['kg', 'lbs', 'reps', 'seconds', 'minutes', 'percentage'] },
        ...
        notes: { type: 'string', example: 'Member showing excellent progress, maintaining workout consistency' },
  ...
  @ApiResponse({ status: 400, description: 'Invalid progress tracking data provided or target value not achievable.' })
  ...
  @ApiResponse({ status: 409, description: 'Duplicate progress record for the same goal and date.' })
  @ApiBody({ 
    type: CreateProgressDto,
    description: 'Progress tracking data including goals, achievements, and performance metrics',
    examples: {
      weight_loss_goal: {
        ...
        value: {
          ...
          notes: 'Member showing excellent progress, maintaining workout consistency',
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves comprehensive list of progress tracking records across all members. Supports filtering by goal type, member, date range, and achievement status. Provides analytics data for business intelligence and member performance insights.'
  })
  ...
  @ApiQuery({ name: 'goalType', required: false, type: String, description: 'Filter by goal type', example: 'weight_loss', enum: ['weight_loss', 'muscle_gain', 'endurance', 'strength', 'flexibility', 'general_fitness'] })
  ...
  @ApiQuery({ name: 'achievement', required: false, type: String, description: 'Filter by specific achievement', example: '5kg_weight_loss' })
  @ApiQuery({ name: 'sortBy', required: false, type: String, description: 'Sort field', example: 'recordedAt', enum: ['recordedAt', 'progressPercentage', 'currentValue', 'createdAt'] })
        ...
        data: {
          ...
          items: {
            ...
            properties: {
              ...
              notes: { type: 'string', example: 'Member showing excellent progress' },
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to view progress tracking records.' })
  findAll() {
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves detailed information about a specific progress tracking record including complete member information, achievement history, and performance analytics. Provides comprehensive view of member fitness journey and goal progress.'
  })
  @ApiParam({ 
    name: 'id', 
    ...
    schema: {
      ...
      properties: {
        ...
        achievements: {
          type: 'array',
          items: { type: 'string' },
          example: ['5kg_weight_loss', '30_day_streak', 'workout_consistency', 'nutrition_goals']
        },
        streakCount: { type: 'number', example: 15 },
        lastUpdateDate: { type: 'string', format: 'date-time', example: '2024-01-15T10:30:00.000Z' },
        estimatedCompletionDate: { type: 'string', format: 'date-time', example: '2024-03-15T00:00:00.000Z' },
        notes: { type: 'string', example: 'Member showing excellent progress, maintaining workout consistency and improved nutrition habits.' },
        recordedAt: { type: 'string', format: 'date-time', example: '2024-01-15T10:30:00.000Z' },
        createdBy: { type: 'number', example: 45 },
        createdAt: { type: 'string', format: 'date-time', example: '2024-01-15T10:30:00.000Z' },
  ...
  @ApiResponse({ status: 400, description: 'Invalid progress tracking record ID format.' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to view this progress record.' })
  @ApiResponse({ status: 404, description: 'Progress tracking record not found or does not exist.' })
  ...
  @ApiOperation({ 
    ...
    description: 'Updates an existing progress tracking record with new values, achievements, or milestone information. Automatically recalculates progress percentage and updates achievement status. Only the creator or authorized personnel can modify records.'
  })
  @ApiParam({ 
    name: 'id', 
    ...
    schema: {
      ...
      properties: {
        ...
        achievements: {
          type: 'array',
          items: { type: 'string' },
          example: ['5kg_weight_loss', '7kg_weight_loss', '30_day_streak', 'nutrition_goals']
        },
        streakCount: { type: 'number', example: 22 },
        notes: { type: 'string', example: 'Updated progress - member exceeded expectations this week' },
        recordedAt: { type: 'string', format: 'date-time', example: '2024-01-15T10:30:00.000Z' },
        updatedAt: { type: 'string', format: 'date-time', example: '2024-01-20T09:15:00.000Z' },
        message: { type: 'string', example: 'Progress tracking record updated successfully' }
      }
    }
  })
  ...
  @ApiBody({ 
    ...
    description: 'Updated progress tracking data - only include fields that need to be updated',
    examples: {
      progress_update: {
      ...
      add_achievement: {
        ...
        value: {
          ...
          notes: 'Member has been very consistent with nutrition and workout schedule'
        }
      }
    }
  })
  ...
  @ApiOperation({ 
    ...
    description: 'Permanently removes a progress tracking record from the system. This action cannot be undone and will affect member goal tracking and analytics. Only admins, the record creator, or users with appropriate permissions can delete progress records.'
  })
  @ApiParam({ 
    name: 'id', 
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        message: { type: 'string', example: 'Progress tracking record deleted successfully' },
        deletedRecord: {
          type: 'object',
          properties: {
            id: { type: 'string', example: 'prog_123456789' },
  ...
  @ApiResponse({ status: 404, description: 'Progress tracking record not found or does not exist.' })
  @ApiResponse({ status: 409, description: 'Cannot delete record - it has associated dependent analytics data.' })
  remove(
    @Param('id', ParseIntPipe) id: number,
    @CurrentUser() user: UserEntity,
  ) {
  ...
  @ApiOperation({
    ...
    description: 'Retrieves all progress tracking records for a specific gym member, including goal progress, achievements, and performance analytics. Provides comprehensive view of member fitness journey and goal completion status.'
  })
  @ApiParam({ 
    name: 'memberId', 
  ...
  @ApiQuery({ name: 'goalType', required: false, type: String, description: 'Filter by specific goal type', example: 'weight_loss', enum: ['weight_loss', 'muscle_gain', 'endurance', 'strength', 'flexibility', 'general_fitness'] })
  @ApiQuery({ name: 'activeOnly', required: false, type: Boolean, description: 'Show only active goals (default: false)', example: true })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Maximum number of records to return', example: 10 })
  @ApiQuery({ name: 'includeHistory', required: false, type: Boolean, description: 'Include progress history data', example: true })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Retrieves all progress tracking records that were created by the currently authenticated user (trainer/staff). Useful for trainers to review their own progress entries, monitor their assigned members, and track their coaching effectiveness.'
  })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number for pagination', example: 1 })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Records per page (max 50)', example: 25 })
  @ApiQuery({ name: 'goalType', required: false, type: String, description: 'Filter by specific goal type', example: 'weight_loss' })
  @ApiQuery({ name: 'memberId', required: false, type: Number, description: 'Filter by specific member ID', example: 123 })
  @ApiQuery({ name: 'activeOnly', required: false, type: Boolean, description: 'Show only active goals', example: true })
  ...
  @ApiResponse({
    status: 200,
    description: 'Successfully retrieved progress tracking records created by current user',
    schema: {
      type: 'object',
      properties: {
        data: {
